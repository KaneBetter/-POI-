...
# 去哪儿热门景点excel文件保存路径
PLACE_EXCEL_PATH = 'data/qunar_place.xlsx'
# 读取数据
DF = pd.read_excel(PLACE_EXCEL_PATH)
# 百度热力图模板
HOT_MAP_TEMPLATE_PATH = 'data/hot_map_template.html'
# 生成的国庆旅游景点热力图
PLACE_HOT_MAP_PATH = 'data/place_hot_map.html'
...
def analysis_sale():
    ...
    place_sale_bar = (
        Bar()
            ...
            .set_global_opts(
            title_opts=opts.TitleOpts(title="湖州旅游热门景点门票销量TOP20"),
...
def analysis_amount():
    ...
    place_amount_bar = (
        Bar()
            ...
            .set_series_opts(label_opts=opts.LabelOpts(position="right"))
            .set_global_opts(
            title_opts=opts.TitleOpts(title="湖州旅游热门景点门票销售额TOP20"),
            yaxis_opts=opts.AxisOpts(name="景点名称"),
...
def analysis_recommend():
    ...
    place_recommend_bar = (
        Bar()
            ...
            .set_series_opts(label_opts=opts.LabelOpts(position="right"))
            .set_global_opts(
            title_opts=opts.TitleOpts(title="湖州旅游热门景点瞎推荐TOP20"),
            yaxis_opts=opts.AxisOpts(name="景点名称"),
